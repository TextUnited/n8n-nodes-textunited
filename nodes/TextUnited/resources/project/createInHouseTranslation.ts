...
const showOnlyForProjectCreateInHouseTranslation = {
	resource: ['project'],
	operation: ['createInHouseTranslation'],
};
...
export const projectCreateInHouseTranslationDescription: INodeProperties[] = [
	{
		...
		required: true,
		displayOptions: {
			show: showOnlyForProjectCreateInHouseTranslation,
		},
	...
	},
	{
		...
		description: 'Languages the content should be translated into. At least one is required.',
		options: [
			{
				...
				values: [
					...
					},
					{
						...
						default: {},
						description: 'People assigned to work on this language',
						options: [
							{
								displayName: 'Member',
								name: 'member',
	...
	},
	{
		...
		displayOptions: {
			show: {
				...showOnlyForProjectCreateInHouseTranslation,
				segmentsMode: ['items'],
			},
		},
		description:
 ...
 * language flagged as staffed in-house rather than by an agency, optionally
 * assigning a team per language. This is a curated, simplified form of
 * project creation: everything not shown above (adaptive/automatic
 * translation, scoring, description, instant translation) is fixed to a
 * sensible default and never exposed.
 *
 * Segments can be provided three ways, via "Specify Segments":
 * - "items": one segment per input item (see
 *   {@link createInHouseTranslationProject}) — the only mode where this
 ...
 *   (see {@link createInHouseTranslationProjectForItem}), evaluated the same
 *   way for every input item — one project created per item, like every
 *   other Create operation on this node.
 */
export async function createInHouseTranslationProject(
 ...
 */
export async function createInHouseTranslationProjectForItem(
	this: IExecuteFunctions,
	i: number,
): Promise<IDataObject> {
